    ...
    ISedeRepository sedeRepository) : IHorarioProfesionalService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (sede.OrganizacionId != empleado.OrganizacionId)
        ...
        CrearExcepcionProfesionalSolicitud solicitud, CancellationToken ct = default)
    {
        ...
        var empleado = await empleadoRepository.ObtenerParaModificarAsync(solicitud.EmpleadoId, ct);
        if (!empleado!.EstaActivo)
            return ResultadoOperacion<Guid>.Fallo(ProfesionalInactivo, TipoErrorOperacion.Conflicto);
        var sede = await sedeRepository.ObtenerParaModificarAsync(solicitud.SedeId, ct);
        if (sede is null || sede.EstaEliminado || !sede.EstaActivo || sede.OrganizacionId != empleado.OrganizacionId)

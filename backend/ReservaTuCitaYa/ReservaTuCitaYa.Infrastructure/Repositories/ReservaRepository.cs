...
public sealed class ReservaRepository(ApplicationDbContext context) : IReservaRepository
{
    ...
    private static IQueryable<ReservaDetalleDto> ProyectarDetalle(IQueryable<Reserva> query) =>
        query.Select(r => new ReservaDetalleDto(
            r.Id, r.OrganizacionId, r.Codigo, r.EstadoReserva.ToString(),
    ...
    public void AgregarReprogramacion(ReprogramacionReserva reprogramacion) => context.ReprogramacionesReserva.Add(reprogramacion);
    public void AgregarCancelacion(CancelacionReserva cancelacion) => context.CancelacionesReserva.Add(cancelacion);
    ...
    public async Task GuardarAsync(CancellationToken ct = default)
    {
        ...
        catch (DbUpdateException ex) when (ex.InnerException is SqlException { Number: 2601 or 2627 or 1205 or 3960 })
        {
            throw new ConflictoPersistenciaException("La reserva entra en conflicto con otra operación concurrente.", ex);
        }
        catch (DbUpdateConcurrencyException ex)
        {
            throw new ConflictoPersistenciaException("La reserva fue modificada por otra operación.", ex);
        }
    }

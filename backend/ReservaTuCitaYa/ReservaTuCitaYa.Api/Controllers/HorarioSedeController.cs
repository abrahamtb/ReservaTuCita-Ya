...
public sealed class HorariosSedeController(IHorarioSedeService service) : ApiControllerBase
{
        ...
        Guid sedeId, CrearExcepcionSedeSolicitud request, CancellationToken ct)
    {
        ...
        return result.EsExitoso
            ? CreatedAtAction(nameof(ObtenerExcepcion), new { id = result.Valor }, result.Valor)
            : Problem(result.Error, result.TipoError);
    }

    [HttpGet("api/excepciones-horario-sede/{id:guid}")]
    public async Task<ActionResult<ExcepcionHorarioDto>> ObtenerExcepcion(Guid id, CancellationToken ct)
    {
        var result = await service.ListarExcepcionesAsync(
            new ExcepcionHorarioFiltroDto(Guid.Empty, null, null, null, 1, 1), ct);
        // Nota: si quieres un GET por Id puro, agrega un método ObtenerExcepcionPorIdAsync al servicio.
        return NotFound();
    }

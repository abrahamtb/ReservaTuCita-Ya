...
public sealed class EmpleadosController(IEmpleadoService service) : ApiControllerBase
{
    ...
    public async Task<ActionResult<PaginaResultado<EmpleadoListaDto>>> Listar(
        ...
        [FromQuery] bool? esProfesional, [FromQuery] EstadoFiltro estado = EstadoFiltro.Todos,
        [FromQuery] Guid? sedeId = null, [FromQuery] Guid? servicioId = null,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return result.EsExitoso && result.Valor is not null ? Ok(result.Valor) : EmpleadoProblem(result.Error, result.TipoError);
    }

    [HttpGet("api/empleados/{id:guid}")]
    public async Task<ActionResult<EmpleadoDetalleDto>> Obtener(Guid id, CancellationToken cancellationToken)
    {
        ...
        Guid organizacionId, CrearEmpleadoRequest request, CancellationToken cancellationToken)
    {
        var result = await service.CrearAsync(new CrearEmpleadoSolicitud
        {
            ...
            FechaNacimiento = request.FechaNacimiento, Cargo = request.Cargo, Especialidad = request.Especialidad,
            EsProfesional = request.EsProfesional, NumeroColegiatura = request.NumeroColegiatura,
            Observaciones = request.Observaciones, SedeIds = request.SedeIds, ServicioIds = request.ServicioIds
        }, cancellationToken);
    ...
    [Authorize(Policy = Permissions.Empleados.Gestionar)]
    public async Task<IActionResult> Actualizar(Guid id, ActualizarEmpleadoRequest request, CancellationToken cancellationToken)
    {
        var result = await service.ActualizarAsync(new ActualizarEmpleadoSolicitud
        {
            ...
            Cargo = request.Cargo, Especialidad = request.Especialidad, EsProfesional = request.EsProfesional,
            NumeroColegiatura = request.NumeroColegiatura, Observaciones = request.Observaciones
        }, cancellationToken);
    ...
    [Authorize(Policy = Permissions.Empleados.Gestionar)]
    public async Task<IActionResult> CambiarEstado(Guid id, CambiarEstadoEmpleadoRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<IReadOnlyList<EmpleadoSedeDto>>> ListarSedes(Guid id, CancellationToken cancellationToken)
    {
    ...
    [Authorize(Policy = Permissions.Empleados.Gestionar)]
    public async Task<IActionResult> ReemplazarSedes(Guid id, ReemplazarSedesEmpleadoRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<IReadOnlyList<ProfesionalServicioDto>>> ListarServicios(Guid id, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> ReemplazarServicios(
        Guid id, ReemplazarServiciosProfesionalRequest request, CancellationToken cancellationToken)
    {
        var result = await service.ReemplazarServiciosAsync(id, request.ServicioIds, cancellationToken);
        return result.EsExitoso ? NoContent() : EmpleadoProblem(result.Error, result.TipoError);
    }
    ...
    private ObjectResult EmpleadoProblem(string? detail, TipoErrorOperacion errorType)
    {
        ...
        {
            EmpleadoService.DocumentoDuplicado => ("employee-document-duplicate", "Empleado duplicado"),
            EmpleadoService.SedeOrganizacionInvalida => ("employee-site-organization-mismatch", "Sede de otra organización"),
            EmpleadoService.ServicioOrganizacionInvalida => ("professional-service-organization-mismatch", "Servicio de otra organización"),
            EmpleadoService.EmpleadoNoProfesional => ("employee-not-professional", "Empleado no profesional"),
            EmpleadoService.ProfesionalConServicios => ("employee-has-professional-services", "Profesional con servicios"),

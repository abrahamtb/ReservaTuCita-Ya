...
public sealed class SedesController(ISedeService service, ICurrentUser currentUser) : ApiControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<SedeListaDto>>> Listar(
        Guid organizacionId, [FromQuery] string? busqueda,
        [FromQuery] EstadoFiltro estado = EstadoFiltro.Todos, CancellationToken cancellationToken = default)
    {
    ...
    [Authorize(Policy = Permissions.Sedes.Ver)]
    public async Task<ActionResult<SedeDetalleDto>> Obtener(Guid id, CancellationToken cancellationToken)
    {
    ...
    public async Task<ActionResult<SedeDetalleDto>> Crear(
        Guid organizacionId, CrearSedeSolicitud request, CancellationToken cancellationToken)
    {
        if (!EsOrganizacionAutorizada(organizacionId)) return Forbid();
        var serverRequest = new CrearSedeSolicitud
        {
            OrganizacionId = organizacionId, Nombre = request.Nombre, Direccion = request.Direccion,
            Telefono = request.Telefono, Correo = request.Correo, Referencia = request.Referencia
        };
    ...
    [Authorize(Policy = Permissions.Sedes.Gestionar)]
    public async Task<IActionResult> Actualizar(Guid id, ActualizarSedeSolicitud request, CancellationToken cancellationToken)
    {
        ...
        {
            Id = id, Nombre = request.Nombre, Direccion = request.Direccion,
            Telefono = request.Telefono, Correo = request.Correo, Referencia = request.Referencia
        }, cancellationToken);

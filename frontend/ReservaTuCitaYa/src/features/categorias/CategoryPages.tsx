...
export function CategoriesPage() {
  ...
  useEffect(() => { const controller = new AbortController(); api.listCategories(organizationId, { busqueda: search, estado: state, pagina: page }, controller.signal).then(setData).catch(e => { if (!controller.signal.aborted && e.name !== 'AbortError') setError(e) }); return () => controller.abort() }, [organizationId, search, state, page])

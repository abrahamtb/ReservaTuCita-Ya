...
export function PagosReservaPage() {
  const { user } = useAuth()
  const canRegister = user?.permisos.includes('pagos.registrar') ?? false
  const canAnnul = user?.permisos.includes('pagos.anular') ?? false
  const canRefund = user?.permisos.includes('pagos.reembolsar') ?? false
  ...
  return <section>
    ...
    <div className="row g-4">
      <div className={canRegister ? 'col-lg-8' : 'col-12'}><div className="card"><div className="card-header fw-semibold">Historial</div><div className="table-responsive"><table className="table mb-0 align-middle"><thead><tr><th>Código</th><th>Fecha</th><th>Método</th><th>Monto</th><th>Estado</th>{(canAnnul || canRefund) && <th />}</tr></thead><tbody>{resumen.pagos.map(item => <tr key={item.id}><td>{item.codigo}</td><td>{item.fechaPago}</td><td>{item.metodoPago}</td><td>{money.format(item.monto)}</td><td>{item.estaAnulado ? <span className="badge text-bg-secondary">Anulado</span> : <span className="badge text-bg-success">Vigente</span>}</td>{(canAnnul || canRefund) && <td className="text-end">{!item.estaAnulado && <>{canAnnul && <button disabled={busy} className="btn btn-sm btn-outline-warning me-1" onClick={() => void annul(item)}>Anular</button>}{canRefund && <button className="btn btn-sm btn-outline-danger" onClick={() => { setReembolsoPago(item); setReembolso(current => ({ ...current, monto: Math.min(item.monto, resumen.totalPagadoNeto) })) }}>Reembolsar</button>}</>}</td>}</tr>)}</tbody></table></div></div>

...
import { ReservationBadge } from '../features/reportes/ReportShared'
  ...
  return <section className="dashboard-page">
    ...
    {!cargando && !error && datos && <>
      ...
      <div className="dashboard-grid">
        ...
        <article className="dashboard-card dashboard-panel"><h2>Reservas por estado</h2>{datos.reservasPorEstado.length ? <div className="dashboard-states">{datos.reservasPorEstado.map(item => <div key={item.estado}><ReservationBadge status={item.estado} /><strong>{item.cantidad}</strong></div>)}</div> : <Vacio />}</article>
      </div>
      <div className="dashboard-grid">

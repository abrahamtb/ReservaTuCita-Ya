...
export const actualizarHorarioSede = (sedeId: string, intervalos: IntervaloHorario[]) => apiRequest<void>(`/api/sedes/${sedeId}/horarios`, { method: 'PUT', body: JSON.stringify({ intervalos: intervalos.map(({ diaSemana, horaInicio, horaFin }) => ({ diaSemana, horaInicio, horaFin })) }) })
...
export const crearExcepcionSede = (sedeId: string, data: ExcepcionRequest) => apiRequest<string>(`/api/sedes/${sedeId}/excepciones-horario`, { method: 'POST', body: JSON.stringify(data) })

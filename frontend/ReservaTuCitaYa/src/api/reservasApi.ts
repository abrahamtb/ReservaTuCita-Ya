...
export const crearReserva = (organizacionId: string, data: CrearReservaRequest) => apiRequest<ReservaCreada>(`/api/organizaciones/${organizacionId}/reservas`, { method: 'POST', body: JSON.stringify({ ...data, horaInicio: time(data.horaInicio) }) })
export const reprogramarReserva = (organizacionId: string, id: string, data: { fechaNueva: string; horaInicioNueva: string; profesionalId?: string | null; recursoId?: string | null; motivo: MotivoReprogramacion; observacion?: string }) => apiRequest(`/api/organizaciones/${organizacionId}/reservas/${id}/reprogramacion`, { method: 'PUT', body: JSON.stringify({ ...data, horaInicioNueva: time(data.horaInicioNueva) }) })

...
export function SedesPage() {
  ...
  useEffect(() => { const controller = new AbortController(); api.listSedes(organizationId, { busqueda: search, estado: state }, controller.signal).then(setItems).catch(e => { if (e.name !== 'AbortError') setError(e) }); return () => controller.abort() }, [organizationId, search, state])
